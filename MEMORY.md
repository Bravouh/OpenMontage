...
- **[license]** OmniVoice: code Apache 2.0; weights pre-trained CC-BY-NC (Non-Commercial). Dự án này là CÁ NHÂN, không kiếm tiền → dùng weights hợp lệ, chỉ cần credit. Nếu sau này bật monetization thì phải đổi model. Chỉ dùng giọng tự thu hoặc voice design, không clone giọng người khác. Package hiện tại 0.2.0 (không phải model v2).
...
- **[chốt]** Máy phát triển = laptop CPU-only (Intel iGPU). Máy công ty GPU NVIDIA chỉ dùng chạy OmniVoice generate hàng loạt ở giai đoạn sau. Toàn bộ build/test/pipeline (phong cách voiceover+ảnh tĩnh+Remotion) chạy được trên CPU.
...
- **[env]** ffmpeg/ffprobe/yt-dlp ở C:\bin, đã vào **USER PATH** (KHÔNG phải System/Machine PATH). User PATH cũng có sẵn đường winget của cả hai (ffmpeg + yt-dlp). Session Claude Code không thấy chúng (CommandNotFoundException) vì app khởi động TRƯỚC khi PATH được cập nhật — môi trường tiến trình là snapshot lúc launch, không tự cập nhật. Cách hết cần prefix: **restart app Claude Code hoặc reboot máy**. Tạm thời (nếu chưa reboot) prefix mỗi lệnh PowerShell bằng: `$env:PATH += ';C:\bin';` <lệnh>.
- **[env]** Read tool KHÔNG đọc được PDF trên máy này (thiếu poppler/pdftoppm → báo "pdftoppm is not installed"). research-director đọc official report bằng **pypdf** (thuần Python, đã thêm vào requirements.txt). Trích text: `PdfReader(path).pages[i].extract_text()`. Report NTSB là scan/OCR nên text hơi nhiễu nhưng đọc được; findings/probable cause ở front-matter, tên phi hành đoàn ở Appendix B (không ở §1.5 body).
- **[lưu]** script-director: dùng **~125 wpm** làm mốc độ dài (40–50 phút ≈ 5.000–6.250 từ). Bài học từ lần chạy UA232 thật: viết "full" một lượt **dễ hụt độ dài nghiêm trọng** — hụt 2 lần (18′ rồi 27′) trước khi đạt 43′. CÁCH ĐÚNG: để **word-count quyết định timestamp** (tính `dur = words/125*60`, tile tuần tự) ngay từ đầu, đừng đặt total_duration trước rồi nhồi chữ sau — sẽ lệch. Muốn đủ dài mà không độn: bung bằng chi tiết THẬT trong report (§2 Analysis, Appendix), mỗi chi tiết gắn source_ref.
